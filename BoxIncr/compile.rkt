...
(define (assert-codepoint arg c)
  (let ((ok (gensym)))
    (seq (assert-integer arg c)
         (Cmp arg (imm->bits 0))
         (Jl (error-label c))
         (Cmp arg (imm->bits 1114111))
         (Jg (error-label c))
         (Cmp arg (imm->bits 55295))
         (Jl ok)
         (Cmp arg (imm->bits 57344))
         (Jg ok)
         (Jmp (error-label c))
         (Label ok))))
       
(define (assert-byte arg c)
  (seq (assert-integer arg c)
       (Cmp arg (imm->bits 0))
       (Jl (error-label c))
       (Cmp arg (imm->bits 255))
       (Jg (error-label c))))
